...
function becomeBig($own, hasClassPicRow) {
    ...
    if (hasClassPicRow) {
        for (var i = 120; i < 720; i += 20) {
            ...
            $own.animate({
                "left": (pictureWallWidth - i) / 2 + "px",
                "top": (pictureWallHeight - i / 1.5) / 2 + "px"
            }, 2);
        }
    } else {
...
function scatter() {
    ...
    $sixDiv.each(function() {
        ...
        $(this).css({
            "left": cssLeft,
            "top": cssTop,
            "background-position": divLeft + "px " + divTop + "px"
        })
    })
}
...
function defineRandom() {
    var randomLeft = Math.floor(680 * (Math.random())) + 30,
        /* 图片left值 */
        randomTop = Math.floor(400 * Math.random()) + 30,
        /* 图片top值 */
